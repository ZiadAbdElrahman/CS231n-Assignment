...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss = np.sum (-np.log(p[range(y.shape[0]),y]))
        loss/= y.shape[0]
        loss += reg *0.5 * (np.sum(w1**2) + np.sum(w2**2))
        pos = [range(y.shape[0]),y]
        dscores = p 
        dscores[pos] -= 1
        dscores /= y.shape[0]
        
        grads['W2'] = h1.T.dot(dscores) + w2 * reg
        grads['b2'] = dscores.sum(axis = 0 ) 
        
        dh1 = dscores.dot(w2.T)
        grads['W1'] = X.T.dot(dh1) + w1 * reg
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers) :     
            ...
            if not self.normalization=='batchnorm':
