...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
#     out, _ = rnn_step_forward(x, , Wx, Wh, b)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        h0, cT, cacheT  = lstm_step_forward(x[:, i, :], h0, cT, Wx, Wh, b)
        h[:, i, :] = (h0)
        cache.append(cacheT)
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, ai, af, ao, ag, next_c = cache[0]
    _, D = x.shape
    ...
    prev_dh = 0
    for t in reversed(range(0, T)):
        dx[:,t,:], prev_dh, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:,t,:] + prev_dh, dc, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = prev_dh
